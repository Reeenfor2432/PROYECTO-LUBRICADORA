...
class manejarCita:
    ...
    def MostrarCita():
        try:
            ...
            cursor = conec.cursor(dictionary = True)  # Crear un cursor para ejecutar consultas
    ...
    def IngresarCita(id_cliente, placa, id_empleado, hora_ingreso,hora_salida, estado,):
        try:
            ...
            resultado = cursor.callproc("sp_insertar_cita", (id_cliente, placa, id_empleado, hora_ingreso,hora_salida, estado, 0))
            id_generado = resultado[-1] 

            conec.commit()  # Valores a insertar
            print(cursor.rowcount, "Registro ingresado con exito")  # Informar si el registro fue exitoso
            conec.close()  # Cerrar la conexión a la base de datos
            return id_generado #Devuelve el id_cita autogenerado para ser utilizado en la vistaGenerarCita
    ...
    def ModificarCita(id_cita, id_cliente, placa, id_empleado, hora_ingreso,hora_salida, estado):
        try:
            ...
            cursor.callproc("sp_actualizar_cita", (int(id_cita), int(id_cliente), placa, int(id_empleado), datetime.strptime(hora_ingreso, "%Y-%m-%d %H:%M:%S"),datetime.strptime(hora_salida,"%Y-%m-%d %H:%M:%S"), estado))
            conec.commit()  # Valores a insertar 
    ...
    def EliminarCita(id_cita):
        try:
            ...
            cursor.callproc("sp_eliminar_cita", (int(id_cita),))
            conec.commit()  # Valores a insertar 
